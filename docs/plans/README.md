...
| 3 | [phase-3-depth.md](phase-3-depth.md) | Months 7–10: inundation + digital twin, SITREPs, rumor tracker, post-disaster mode, mobile app, IoT — and the microservice split | 🟡 Milestones 1–3 built (bathtub inundation model over a real Copernicus DEM extract, wired into alerts + routing; hourly auto-SITREPs in NDMA format from verified DB state, with real hotspot-movement tracking and audit-linkable filing; harmonic-trend sensor forecasting + hazard-front propagation pre-alerts, both with a real validation loop and a public per-location accuracy endpoint) |
