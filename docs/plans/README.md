...
| 3 | [phase-3-depth.md](phase-3-depth.md) | Months 7–10: inundation + digital twin, SITREPs, rumor tracker, post-disaster mode, mobile app, IoT — and the microservice split | 🟡 Milestones 1–5 built + milestone 6's IoT pilot (bathtub inundation model over a real Copernicus DEM extract, wired into alerts + routing; hourly auto-SITREPs in NDMA format from verified DB state, with real hotspot-movement tracking and audit-linkable filing; harmonic-trend sensor forecasting + hazard-front propagation pre-alerts, both with a real validation loop and a public per-location accuracy endpoint; embedding-clustered rumor tracker with analyst-approved corrections, plus per-tier/language/channel alert drafting; Expo mobile app with an offline-first queue that preserves observation times and dedupes retries, plus Mark Safe check-ins; LoRaWAN IoT pilot over a real EMQX MQTT broker feeding the shared sensor/anomaly path — a node is just a station with provider `iot`, CoastSnap half deferred) |
